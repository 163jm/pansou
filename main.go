...
func main() {
	// 精简日志输出：屏蔽各插件/内部模块通过标准库log(Printf/Println等)打印的调试信息。
	// 启动摘要信息使用fmt.Print*直接输出，不受影响。
	// 注意：log.Fatalf等致命错误也会被一并屏蔽（进程仍会退出，只是看不到原因），
	// 排查启动失败等问题时，设置环境变量 VERBOSE_LOG=true 临时恢复完整日志。
	if os.Getenv("VERBOSE_LOG") != "true" {
	...
	if config.AppConfig.AsyncPluginEnabled {
		...
		if len(plugins) > 0 {
			if config.AppConfig.EnabledPlugins == nil {
				fmt.Printf("未设置 ENABLED_PLUGINS，默认启用全部插件 (%d个):\n", len(plugins))
			} else {
			...
			// 区分不同的情况
			if config.AppConfig.EnabledPlugins != nil && len(config.AppConfig.EnabledPlugins) > 0 {
